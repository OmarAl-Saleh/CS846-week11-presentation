...
def _normalize_user_agent(user_agent: str) -> str:
    """Normalize UA to keep only device-relevant ext flags (touch/tablet_mode).

    This drastically reduces unique UA cardinality while preserving
    device/browser hints used by the heuristics.
    """
    lower = user_agent.lower()
    # fast search for '; ext=' or ';ext=' (case-insensitive already via lower)
    idx = lower.rfind("; ext=")
    if idx == -1:
        idx = lower.rfind(";ext=")
    if idx == -1:
    ...
    # slice ext portion from original string to preserve casing in the prefix
    # compute start index of ext value
    start = idx + (6 if lower[idx:idx+6] == "; ext=" else 5)
    ext = user_agent[start:]
    # split on commas and whitespace without regex
    parts = [p.strip().lower() for p in ext.replace(",", " ").split() if p.strip()]
    keep = [p for p in parts if p in ("touch", "tablet_mode")]
    if not keep:
        return user_agent[:idx]
    uniq = sorted(set(keep))
    return user_agent[:idx] + "; ext=" + ",".join(uniq)


@lru_cache(maxsize=50_000)
def _parse_user_agent_cached(normed_ua: str) -> Tuple[str, str]:
    """Actual parsing logic, cached on a normalized UA string."""
    ua = normed_ua
    lower = ua.lower()
...
def parse_user_agent(user_agent: str) -> Tuple[str, str]:
    ...
    Wraps the cached parser but first normalizes high-cardinality ext
    fields so caching is effective while preserving semantics.
    """
    norm = _normalize_user_agent(user_agent)
    return _parse_user_agent_cached(norm)
...
def compute_client_mix(events: Sequence[Event]) -> Dict[str, Counter]:
    ...
    # Aggregate counts per unique user_agent so we parse each UA only once.
    ua_counts: Counter = Counter(e.user_agent for e in events)
    for ua, cnt in ua_counts.items():
        device, browser = parse_user_agent(ua)
        device_counts[device] += cnt
        browser_counts[browser] += cnt
